...
- **Hard failures** — everything else stops with `{stepId, intent, expected, observed, screenshot}`. Session expiry is deliberately fatal-not-recoverable: flow state is lost, and whether to re-run from the entry point is the *caller's* retry decision, not something replay should improvise mid-flow.
...
**Multi-tenant reuse.** Many tenants run the same vendor product configured differently, so an artifact records against the *vendor app profile* (`appId`), not a tenant. The design (not built): a base artifact plus thin per-tenant **overlays** — entry URL, branding-driven locator overrides for specific steps, parameter defaults — layered at load time, so one recorded capability serves N tenants and only deltas are maintained. Drift management is telemetry from replays: which locator tier resolved, checkpoint pass rates, per tenant. Tier degradation → flag for review; hard failure → escalation + re-record of the affected step for that overlay. The `version` + `status` fields give overlays a review lifecycle.
...
- **Tenant overlays** — designed (§4), not implemented; one app profile stands in.
...
Next, in order: (1) expose the catalog as a tool-calling surface (each artifact's parameters/outputs already are a function schema, so this is mostly transport); (2) assisted fallback — on locator failure, a bounded single-step LLM repair, policy-checked and recorded as a new draft version; (3) tenant overlays with tier-degradation telemetry; (4) a real operator console over the CDP seam.
