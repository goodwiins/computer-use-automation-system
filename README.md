...
Five MERIDIAN capabilities have accepted recording/replay pairs: sign-on, member inquiry, member record, open share and funds transfer. Contact update and supervisor hold remain incomplete. See [live evidence](docs/meridian/live-evidence.md), including the separately recorded unknown posting. Existing mock fixtures are not a substitute for live evidence.

Funds transfer can be triggered from chat with a server-enforced two-step confirmation: the assistant first calls `prepare_funds_transfer`, which validates the five facts against the capability contract and returns a preview card without starting anything, and the run starts only after the user explicitly confirms that preview. The confirmation is held server-side, bound to the exact prepared facts and a ten-minute expiry, and is consumed by the first accepted start. Final posting still requires operator approval at the risk gate. The other guided writes (open share, contact update, supervisor hold) remain form-directed: chat points to the operation form card and never starts them.
