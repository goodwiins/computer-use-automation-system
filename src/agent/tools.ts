...
- Extract only the outputs named in the goal. Verify eligibility or intermediate pages with assert, never with extract; an undeclared output name is refused and cannot complete the recording.
- A funds-transfer recording has one scalar output, confirmation: extract it with a pattern from the receipt text. The runtime proves the posting with a fresh member read-back of share balances; never record a transaction table or copy values into done.
