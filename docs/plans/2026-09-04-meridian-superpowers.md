...
- Task 5 — partial: discovery `a06406ce-c425-4cfb-bb61-4e23b73f8845` succeeded at source `745ef645ae48730e769e6fc639ec4f71739d23e8`, with one human-approved posting, evaluator pass and independent resulting-state verification. Its genuine 22-step artifact remains `draft`: four Important promotion findings are unresolved. No transfer replay occurred; fully accepted capability coverage remains `3/7`.
...
- Task 5 remediation/promotion/replay, Tasks 6–8 and the remaining live portions of Task 9 remain pending: the successful first transfer does not establish a reusable verified write artifact, separate replay, the other write capabilities, operation-specific unhappy paths, integrated dashboard/chat rehearsal, same-browser repair, approval/handoff keyboard operation or final delivery.
...
**Files:** Read the frozen artifact, existing private discovery evidence, `src/surface/browser.ts` (`inspectControl`, `readTable`), `src/runtime/profile.ts` (`LiveControl`), and the approved member-record extraction. Record only sanitized structural conclusions in `docs/meridian/live-evidence.md` and `docs/meridian/evidence/funds-transfer/`.

- [ ] Verify the frozen hash and source above; preserve the existing dirty artifact, test and evidence edits before any branch integration. Refresh PR #37 at `745ef645ae48730e769e6fc639ec4f71739d23e8` and `dev`, including PR #38's merge `5bc21fe9728b59ce6c9cf2013c48b85646eaf179`, before reconciling the acceptance branch. Coordinate with current branch owners; do not repeat Tasks 1–3 or overwrite concurrent work.
- [ ] Build a field map from the actual recorded pages: selected member route, share ID/status/balance columns, native form field names, visible review labels, confirmation source, transaction detail labels, table target and header-cell structure. Use invariant structural selectors plus exact row/value comparisons in TypeScript. Do not interpolate member/share/memo values into CSS; retain recorder `safeCss` rejection of parameter-dependent selectors.
- [ ] Verify each required field exists and has one unambiguous interpretation. The repository's general `td`-header guidance is not proof of this transaction table's shape. If the retained evidence is insufficient, leave that mapping unverified; after implementation resumes, obtain a separately scoped read-only observation where possible. Never submit a transfer just to inspect a result table.
- [ ] Use the verified member/input/review map for runtime comparisons. The saved result labels/header layout are unverified: define the typed output contract below without guessing physical result selectors. A new complete recording in Task 5c must observe and verify those selectors; missing values may never be filled from submitted parameters and called extracted outputs.

### Task 5b: Enforce the semantics in the shared runtime, offline first

**Implementation checkpoint:** Runtime implementation is committed on `codex/meridian-transfer-runtime`; the implementer reports 112 focused tests and 241 full-suite tests passing, plus typecheck, artifact validation and diff checks. Independent review and hosted checks remain pending. This does not approve the old draft or verify live result layout.
...
- [ ] Write failing behavioral regressions before changing the runtime. Use synthetic values in existing fixtures; include a control that passes and table-driven mutations for every gate above. The minimum pure comparison check is:
...
Expected: focused behavioral checks pass after the fixes. The catalog test still rejects the unapproved transfer draft. Do not replace these regressions with assertion counts or checks of contract literals written by the same constructor. Have a fresh reviewer check all four findings and the shared call paths before proceeding to live work.

- [ ] Deliver Tasks 5a–5b through the separate transfer-runtime PR against `dev`, using the PR gates above. Keep the pending artifact/catalog changes in their own worktree. Begin Task 5c on the resulting reviewed `dev` baseline after integration; no live posting is authorized by the runtime merge.
...
| Exact transfer eligibility, review facts, completion details, table headers — pending | Task 5a–5c regression and review gates pass; no wrong member/share/amount/memo accepted; no header returned as transaction data |
...
- Remaining transfer interfaces checked against `745ef645ae48730e769e6fc639ec4f71739d23e8`; Task 5b's additions are implemented on the separate runtime branch based on `5bc21fe9728b59ce6c9cf2013c48b85646eaf179`, pending independent review and hosted checks. Future artifact work must use the reviewed integrated baseline.
