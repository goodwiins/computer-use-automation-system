...
**Tech Stack:** Node 22, TypeScript, Express, Playwright, Zod, OpenAI/Azure and Vitest for the accepted runtime. The user separately selected Vercel AI SDK for the conversational backend and [assistant-ui](https://www.assistant-ui.com/) for the final chat component map. That UI work is active in an isolated sibling task; explicit package versions and visual/interaction scope remain an integration gate.
...
**Execution status:** Current integrated `dev` is `245ba838499212ee36ee556e0a6de7c671b01f76`. PRs #43–#47 delivered the offline insufficient-funds, loader, review association, Terminal approval and trusted-metadata repairs; PR #49 added canonical discovery preflight and promotion-binding checks. Their reviewed head and merge gates passed. Fresh sign-on, inquiry, member-record and real-model caller API reads passed. Later transfer attempts reached the approval boundary but ended as terminal failures with zero mutation intents; the latest `e76197b9-1c6f-46cb-965d-094f60f93992` timed out after five minutes. No new candidate, promotion or replay exists. Preserve the earlier successful transfer discovery/draft and each adverse request identity. Every live post still requires a new human decision on current facts.

**Assignment §2 supplement:** The [target-application implementation plan](2026-09-04-meridian-target-application.md) maps all seven functions and exceptional states to existing PRs and remaining work. Task A is complete through PR #43, merged into `dev` at `4f84b9fbcb9b5a1fc09cd05611277b3357561728`: discovery/replay now return terminal `INSUFFICIENT_FUNDS` before intent while preserving post-intent uncertainty. Its offline checks do not replace the remaining live gates.

## Global Constraints

- “All seven required functions must work, including approved posting.”
- Current accepted runtime decision: one server process, one active run, one visible browser per run and filesystem persistence, with a concurrency ceiling. The existing static dashboard remains authoritative while the independently authorized Vercel AI SDK/assistant-ui work is reviewed and integrated.
...
## Current integrated baseline — September 5, 2026
...
The acceptance worktree delivered [PR #37](https://github.com/goodwiins/computer-use-automation-system/pull/37) at reviewed head `745ef645ae48730e769e6fc639ec4f71739d23e8`; it is integrated into `dev` at `480b252ab60edc77aff1bc37f6cd08ba9645f8d1`, whose workflow `33919679746` / producer `101174884826` passed. [PR #39](https://github.com/goodwiins/computer-use-automation-system/pull/39) and [PR #41](https://github.com/goodwiins/computer-use-automation-system/pull/41) delivered transfer/runtime lifecycle repairs through `aa90387244be07b9955b8b5b83eacf4b9f3058a1`. PRs [#43](https://github.com/goodwiins/computer-use-automation-system/pull/43), [#44](https://github.com/goodwiins/computer-use-automation-system/pull/44), [#45](https://github.com/goodwiins/computer-use-automation-system/pull/45), [#46](https://github.com/goodwiins/computer-use-automation-system/pull/46), [#47](https://github.com/goodwiins/computer-use-automation-system/pull/47) and [#49](https://github.com/goodwiins/computer-use-automation-system/pull/49) are integrated in order through current `245ba838`; their reviewed head and merge checks passed. PR #49 head workflow `33952285341` / producer `101269200295` and merge workflow `33952963465` / producer `101271025160` passed. Exact source gates are in [the checkpoint report](../meridian/report.md#architecture-and-delivered-runtime). These runtime integrations do not promote the transfer draft, accept a write capability or raise live coverage above `3/7`.
...
- PRs #43–#47 and #49 — delivered in `dev` through `245ba838`: insufficient-funds classification, CLI loader inspection, sibling transfer review mapping, Terminal approval guidance, trusted evidence metadata and canonical discovery preflight/promotion bindings. PR #49 merge workflow `33952963465` / producer `101271025160` passed. The latest three approval-boundary attempts remained terminal failures with evaluator pass and zero mutation intents.
- Current audit — open: discovery still does not run replay's profile detectors, so known business, fatal or recoverable pages can reach model work or lower-layer success. This is not an artifact or POST bypass claim. The detector parity repair and EXC-02 through EXC-07 acceptance remain pending.
...
- One genuine funds-transfer draft exists locally. The four shared runtime findings below are delivered, but the draft remains unpromotable because complete replacement recording, observed result extraction and separate replay evidence are absent. Three other write artifacts and all four write replay gates remain open.
...
Profile and contract corrections reuse `config/app-profiles/meridian.json`, `src/runtime/contracts.ts` and the existing shared execution paths identified in Task 5, with focused regressions in existing tests. Keep the current dashboard, Express/shared `InvocationService` path, server approval, authentication and operator controls authoritative during capability work. The isolated Vercel AI SDK/assistant-ui implementation is independent of this transfer repair and must adapt to those boundaries.
...
| Final chat UI integration | After capability/runtime/API acceptance | Integrate the user-authorized Vercel AI SDK/assistant-ui sibling work only after its exact package versions, visual/interaction scope, tests and PR are reviewed; preserve auth/operator controls and reuse accepted evidence/request identity |
...
- [x] Delivered Tasks 5a–5b through transfer-runtime PR #39 and delivered Task A plus the CLI/review/approval/evidence-metadata and discovery-contract repairs through PRs #43–#47 and #49 at current `dev` `245ba838499212ee36ee556e0a6de7c671b01f76`. Keep artifact/catalog changes in their own worktree; no runtime merge authorizes a live post.
...
- [ ] Stop CLI discovery before starting the existing server. Through the existing caller API/operator controls, request an explicitly selected new transfer with current member/share/balance facts and its saved replay key. Require a separate human decision on the exact review facts, then verify that run's output correspondence and fresh resulting-state deltas. The independently authorized UI task does not replace this server-side acceptance path. Do not reuse the discovery's balance change as replay evidence.
...
For the status column, record discovery and replay separately (`D` and `R` below): a pre-intent discovery stop or escalation is `D: stopped` or `escalated` with journal `failure`; a pre-intent replay business detector is `R: business_outcome` with journal `business_outcome`; a pre-intent replay fatal or policy/validation failure is `R: failure(<code>)` with journal `failure`; and post-intent discovery is `D: stopped(POST_OUTCOME_UNKNOWN)` while post-intent replay is `R: failure(POST_OUTCOME_UNKNOWN)`, with journal `POST_OUTCOME_UNKNOWN` in both cases. With Task A integrated, a validated underfunded request terminates discovery and replay as `business_outcome / INSUFFICIENT_FUNDS`, with journal `business_outcome`. A post-intent run with verified completion remains success; the unknown branch applies only to a failed or unverified completion.
...
| insufficient funds | Freshly observe a valid underfunded source; Task A source `6ab82fe0cde9cc9dae64e5c69ca019753c42ad89` passed 131 focused and 274 full offline tests. This is not live acceptance. If the target itself supplies a native rejection, record its observed `POST <form action>` rather than forcing an injection | Guard result is pre-intent only after identity, row, status and money checks; a native rejection visible only after mutation intent is post-intent | Offline Task A result: pre-intent `D business_outcome(INSUFFICIENT_FUNDS)` and `R business_outcome(INSUFFICIENT_FUNDS)` → journal `business_outcome`. Post-intent: `D stopped(POST_OUTCOME_UNKNOWN)` and `R failure(POST_OUTCOME_UNKNOWN)` → journal `POST_OUTCOME_UNKNOWN`. Live gate remains incomplete. | 0 pre-intent; 1 post-intent |
...
The user has authorized an isolated sibling implementation using Vercel AI SDK for the conversational backend and [assistant-ui](https://www.assistant-ui.com/) stock chat/tool-result components. The controller verified that commits `7eeb447` and `6353741` exist only in a separate, unpublished local UI worktree; they are external to PR #48 and cannot be checked out from its remote refs. UI tests, a UI PR and integrated acceptance remain unverified. Final integration remains after Tasks 5–8 and non-UI Task 9 acceptance and must preserve Express, the shared `InvocationService`, server approval, the existing dashboard/auth/operator controls and authoritative run ID/status/result/evidence. Select explicit package versions and close visual/interaction scope at that gate. A genuine new post still needs its own human approval and no post exists solely for a UI checkbox.
...
- Scope checked against the replacement design; existing implementation is reused. The four transfer runtime findings are delivered, while the historical draft remains blocked on complete replacement recording/result extraction and replay evidence. Three other write recordings and all four write replay gates remain open.
