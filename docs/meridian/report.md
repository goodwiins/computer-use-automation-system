...
The remaining current audit finding is outside this documentation worktree's `8e625cc` source. The discovery-input audit found missing canonical public inputs can reach runtime/model work and can be compiled as literals for non-transfer capabilities; the unconditional transfer post backstop still blocks an unbound canonical transfer post. Its shared preflight/promotion repair is queued from the PR #47 baseline. PR #47 does not rewrite old private evidence.
...
The user separately authorized parallel implementation of a Vercel AI SDK conversational backend and an [assistant-ui](https://www.assistant-ui.com/) component map. That isolated task has not supplied a verified PR or commit to this checkpoint. Final integration must preserve Express, the shared `InvocationService`, server approval, dashboard/auth/operator controls and authoritative run ID/status/result/evidence. Package versions and visual/interaction scope remain a final-phase gate; the UI work does not satisfy any of the seven live capabilities.
