...
## Accepted funds-transfer recording and replay

Both halves ran on September 8 through the CLI against the read-back contract, each with a fresh filesystem journal and a separate typed Terminal approval. [Discovery](evidence/transfer-pair-sep8/discovery.json) `4a01a974-4a07-460a-aff1-1dc3b112c569` at source `e1fcb35` recorded 19 steps: sign-on, member inquiry, exact member selection, Funds Transfer from that record, native review, one personal approval, one dispatch. The post-dispatch receipt extract failed once and succeeded on retry; the `done` read-back navigated to the member record, read the shares table and verified the source balance fell and the destination balance rose by exactly the requested amount with every other share unchanged. Signed evaluation passed with 20 attempts, one mutation intent and no violations. Review removed one inquiry checkpoint whose text embedded the member's name and share count; the original draft hash is retained and no steps were invented. The artifact was promoted with `replay --approve` and passes `validate`.

[Replay](evidence/transfer-pair-sep8/replay.json) `abdcbf79-e431-40fe-83b3-c129ab1fd4e2` at `65ccffe` executed the approved artifact deterministically with `--attended`: 19 steps, no step retries, a separate personal approval, one dispatch, read-back verified, evaluator pass. A separate read-only inquiry afterwards showed the two shares at exactly the expected balances. [An unattended replay](evidence/transfer-pair-sep8/unattended-abort.json) `54686e06` of the same artifact reached the irreversible step, was refused by the gate and aborted before dispatch with zero intents. [Rerunning the identical command with the same key](evidence/transfer-pair-sep8/dedupe.json) returned the existing run and changed nothing. Live acceptance is therefore **5/7**. The integrated API/operator transfer rehearsal (DEMO-01) remains open because the service journal holds an unknown transfer row from earlier the same day.
...
## September 8 transfer recording on the read-back contract

Two CLI discoveries ran against the rebased read-back source with a fresh filesystem journal. `defd6969-b717-4b2f-915f-85d410e06070` reached the transfer review and the posting approval gate (17 actions), then aborted with `RUN_ABORTED` because the invoking shell had no TTY; zero mutation intents, journal `failure`. `77b6b9e5-b6e8-4cc5-899a-f6cff73a0c85` reached the same gate, received a typed Terminal approval, dispatched once at attempt 17, extracted the receipt (attempt 18), and the `done` read-back navigated to the member record and extracted the shares table (attempts 19–20) before completion validation failed. The model retried twice; a failed post-dispatch `navigate` then finished the run `POST_OUTCOME_UNKNOWN`. Journal state is unknown and stays so by design.

A separate read-only inquiry immediately afterwards showed the source share at exactly the pre-approval balance minus the requested amount and the destination at exactly the pre-approval balance plus it, so the posting itself succeeded. The evidence did not retain why completion failed: `discovery.completion` carried no category and output names are withheld. Both runs recorded one `extract` before entering the transfer, and completion requires outputs to be exactly `confirmation`, which is the most probable cause. Two repairs follow from this: `extract` now refuses any output name not declared by the capability contract, and `discovery.completion` records a safe `completionFailure` category (`outputs`, `state`, `frame`, `other`). Neither run produced a candidate artifact; the transfer remains unaccepted and this posting must not be repeated.
...
The transaction result's label set, row order, header shape and confirmation/detail relationship remain unverified, and September 8 showed that a receipt-row contract cannot be established safely: the receipt is read only after the posting, so every extraction miss costs a real transfer. Task 5b therefore no longer requires a canonical transaction row. The transfer contract is now one scalar `confirmation` output extracted from the receipt plus a fresh member read-back after dispatch, in which the source balance must fall and the destination balance must rise by exactly the requested amount while every other share is unchanged. This mirrors the accepted open-share pattern and is the resulting-state verification Task 5d already demanded. The reviewed runtime does not promote the draft or prove replay success; a new complete recording is still required before Task 5c.
