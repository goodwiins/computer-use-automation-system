...
Status: acceptance in progress. Three of seven live LLM-discovered capabilities are reviewed and replayed: sign-on, member inquiry, and member record. Four write capabilities and verified approved postings remain incomplete. [Live evidence](live-evidence.md) records the completed reads, the genuine dashboard read, the current caller chat/API read and the separate unknown posting; never retry that posting. The published documentation/test checkpoint is `ca5d99a21e7274445eb119a71bc8c61f548fa9a7`; the dashboard timing repair's new head awaits controller checks. All new PRs target `dev`, while `master` remains production and untouched.
