...
**Fix status (2026-08-22, commit 62a5ccc):** C1, C2 (record-time floor),
C3, H1, H2, H3, H4 are fixed with regression tests (75 total).
M1 partially fixed (Surface signatures carry risk type-safely; guardedness
itself is still comment-enforced). Open: M2–M5, L1–L3, and artifact
signing for C2/M4.

## Critical

### C1 — `--overlay` + `--approve` corrupts the base artifact [FIXED 62a5ccc]
...
### C2 — Risk class is trusted from artifact JSON; approval is one unauthenticated flag [PARTIALLY FIXED 62a5ccc — record-time risk floor; signing still open]
...
### C3 — Protocol-relative URL bypasses origin allowlist [FIXED 62a5ccc]
...
| # | Where | Problem |
|---|-------|---------|
| H1 [FIXED] | `cli.ts:131`, `schema.ts:107` | Path traversal: `--name ../../foo` writes outside `artifacts/`. `id`/`name` are unconstrained `z.string()` |
| H2 [FIXED] | `browser.ts:49-55` | `currentUrl()` prefers a frame named `workarea`, else `frames[0]`. Used by `gate()` default and `assertStillInBounds()` — post-navigation bounds check can read a stale/wrong frame URL and pass |
| H3 [FIXED] | `logger.ts:31-39` | Screenshots bypass the Redactor entirely — sensitive values rendered on-page persist in evidence PNGs while logs are masked |
| H4 [FIXED] | `executor.ts:100-102` | Detector recovery clicks execute config-supplied targets at default `'read'` risk. Tampered app-profile detector = free unattended click on any control |

## Medium

| # | Where | Problem |
|---|-------|---------|
| M1 [PARTIAL] | `executor.ts:191,198,205` | `(surface.click as …)` casts drop the `risk` param from the `Surface` interface. Composing `runReplay` with a raw `BrowserSurface` silently disables enforcement — guardedness is comment-only, not type-enforced |
