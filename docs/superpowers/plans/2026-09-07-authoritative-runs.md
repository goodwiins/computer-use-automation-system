...
- [ ] First integrate the reviewed and published PR97 head `989474098fa6be468cb8d8a0799272ed97a086dc` (includes the reviewed uncertain-submission correction) by an explicit merge, not copying its implementation. Parent/UI preflight found service/http identity+availability conflicts: keep B1 principalRole/principalKey and subjectId, add availability, preserve six-argument `invoke(..., role, lookupOnly=false)` shared normalized lookup before reserve, keep live-only finishedAt. Keep one current PR97 browser identity test where duplicate placement conflicts. Controller verified this exact published head before dispatch; hosted CI is tracked separately. No provider/dialog edits; session.tsx changes are only Awaited Run/Availability aliases (catalog stays sync).
...
- [ ] Add the parent/UI-agreed model-free chat recovery route: authenticated `GET /api/chat/request` with original `Idempotency-Key` header. Validate the key; demote via existing `callerPrincipal` while retaining subject, `await journal.findRequest(principalKey(principal), key)`, then authorize the safe run projection. Return exactly `{ kind: 'run', runId, capability, state }`. Invalid key400; missing/foreign404 with fixed message; storage unavailable503 with fixed message. No model creation/call, invoke/reserve, bindReference or other write, raw args/results/interventions/private-child details. Root capability tools use the exact header key; run_status aliases that key; linked identity child has its own `member-identity:<parentRunId>` key. A200 proves a run binding only, never whole stream/linked-child settlement. Test direct and status-alias lookup, UNKNOWN, subject/operator demotion, invalid/missing/foreign/storage errors, zero execution/writes and unchanged identities with filesystem and real PG fixtures. UI owns retained original attempt/shared submission hold, keeps404/503 unconfirmed, and must still check authoritative run/availability before new intent.
- [ ] Fix the confirmed private-child intervention liveness issue using the existing history/GET/decision contract. Include a private child in history only for an authorized operator while that child's approval is pending. Centralize the private pending projection in get: preserve only original pending id/expiresAt and request.kind, pinned capability, goal `Complete the linked identity check.`, reason `Linked identity check needs operator attention.`, url `(unavailable)`; omit action/step/screenshot and raw request fields. Other private inputs/results remain withheld and private terminal history stays hidden. Existing decision validation still targets exact run/id and cannot auto-retry, clear active or infer expiry. A generic private card without exact public action facts cannot enable new mutation approval: reject `approve` for a private child at the shared service decision boundary (fixed409), while explicit abort and kind-valid replay retry retain existing exact pending-ID checks. Test direct HTTP approval as well as the rendered card; UI disabling alone is insufficient.
