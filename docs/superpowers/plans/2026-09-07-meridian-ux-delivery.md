...
| 5 | Published [PR100](https://github.com/goodwiins/computer-use-automation-system/pull/100), hosted-review repair head `3d73ddf` | Explicitly depends on PR99. Three hosted P2 findings were reproduced and repaired: resolved private review runs no longer re-enter watched history, history uses one journal list projection, and direct `lookupOnly` survives artifact removal/upgrades through an exact raw-free recovery HMAC while rejecting status aliases. Scoped/final correction reviews approved. Local exact-head full CI passed 912 tests across 35 files, both typechecks and production build; smoke 19, validation and diff checks passed. Corrected exact-head hosted CI/reviews pending. Cross-person grants remain unavailable. |
