...
| 5 | Published [PR100](https://github.com/goodwiins/computer-use-automation-system/pull/100), latest security-repair source head `37d401b` | Explicitly depends on PR99. The first three hosted P2 findings were repaired at `3d73ddf`: resolved private review runs no longer re-enter watched history, history uses one journal list projection, and direct `lookupOnly` survives artifact removal/upgrades through an exact raw-free recovery HMAC while rejecting status aliases. A subsequent hosted security advisory found linked-run event pages still fanned out journal reads; `37d401b` adds bounded deduplicated batch lookup, authenticated snapshot projection and non-queueing per-subject backpressure. Both repair ranges were independently approved. Local exact-source-head full CI passed 919 tests across 36 files, both typechecks and production build; smoke 19, validation and diff checks passed. Corrected exact-final-head hosted CI/reviews pending. Cross-person grants remain unavailable. |
| 6 | assistant-ui persistent conversation adapter: isolated implementation checkpoint paused before integrating the corrected Unit 5 dependency | [PR96](https://github.com/goodwiins/computer-use-automation-system/pull/96), reviewed head `1e029eb`, source CI782 and exact hosted34084793710 passed; real PG/journal restore30/30 accepted. Unmerged. Installed-interface and real-PG/browser acceptance audits are complete; only an intentional red controller test is preserved while the corrected PR100 head is integrated. Safe events only, no replay on restore |
...
- [x] Run existing duplicate/expired/response-loss/unknown tests with the new review entry path plus full CI. Independent review and a separate PR complete this unit; no real decision is clicked by the implementation task.
